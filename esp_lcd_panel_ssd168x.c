...
static const char *const mode_strings[8] = {"000", "001", "010", "011",
                                            "100", "101", "110", "111"};
#endif

static esp_err_t set_ram_params(epaper_panel_t *epaper_panel, int x, int y,
                                int xe, int ye, uint8_t em, bool swap_xy) {
  ram_params_t *p = &(epaper_panel->_ram_params);
  DLOG(TAG, "[%s]", __func__);
  DLOG(TAG, "[%s] 0 x=%d, xe=%d | y=%d, ye=%d | em=%02x, swapxy:%d", __func__,
       x, xe, y, ye, em, swap_xy);
  ...
  int16_t w = xe - x;
  int16_t h = ye - y;
  // int16_t wb = (w + 7) >> 3u; // calculate source size in bytes (w+7)/8 x -=
  // (x & 0xff);                                                     // align to
  // x % 256 w = p->wb << 3u; // byte boundary p->wb * 8
  p->x = x < 0 ? 0 : x; // limit to 0
  p->y = y < 0 ? 0 : y; // limit to 0
  p->w = x + xe < epaper_panel->width
             ? w
             : epaper_panel->width - x; // limit to panel width
  p->h = y + h < epaper_panel->height
             ? h
             : epaper_panel->height - y; // limit to panel height
  p->xe = p->x + p->w;
  p->ye = p->y + p->h;
  // p->dx = p->x - x;
  // p->dxe = p->xe - xe;
  ...
  // p->x -= p->x % 8;
  DLOG(TAG, "[%s] 1 x=%d, xe=%d, w=%d | y=%d, ye=%d, h=%d", __func__, p->x,
       p->xe, p->w, p->y, p->ye, p->h);
  SSD168X_RETURN_ON_FALSE((p->w > 0) && (p->h > 0), ESP_ERR_INVALID_ARG, TAG,
                          "Invalid x,y,xe,ye");
  ...
  DLOG(TAG, "[%s] 2 x=%hu, xe=%hu, w=%d | y=%hu, ye=%hu, h=%d", __func__, p->x,
       p->xe, p->w, p->y, p->ye, p->h);
  DLOG(TAG,
       "[%s] 2 xs_d8=%hhu, xe_d8=%hhu, ys_m256=%hhu, ys_d256=%hhu, "
       "ye_m256=%hhu, ye_d256=%hhu | buffer_size=%u, ram_mode=%02x",
       __func__, p->xs_d8, p->xe_d8, p->ys_m256, p->ys_d256, p->ye_m256,
       p->ye_d256, p->buffer_size, p->ram_mode);
  return ESP_OK;
}
...
static void epaper_driver_gpio_isr_handler(void *arg) {
  ...
  if (epaper_panel->epaper_refresh_done_isr_callback.callback_ptr) {
    (epaper_panel->epaper_refresh_done_isr_callback.callback_ptr)(
        &(epaper_panel->base), NULL,
        epaper_panel->epaper_refresh_done_isr_callback.args);
  }
}

esp_err_t epaper_panel_register_event_callbacks_ssd168x(
    esp_lcd_panel_t *panel, epaper_panel_callbacks_t *cbs, void *user_ctx) {
  ILOG(TAG, "[%s]", __func__);
  if (panel == NULL) {
...
esp_err_t epaper_panel_set_custom_lut_ssd168x(esp_lcd_panel_t *panel,
                                              const uint8_t *lut, size_t size) {
  ILOG(TAG, "[%s]", __func__);
  if (panel == NULL) {
    ESP_LOGE(TAG, "%s", drv_msg[3]);
    return ESP_ERR_INVALID_ARG;
...
static esp_err_t epaper_set_display_sequence(esp_lcd_panel_io_handle_t io,
                                             uint8_t mode) {
  ILOG(TAG, "[%s] mode: %02x", __func__, mode);
  DMEAS_START();
  if (!mode)
    mode = SSD168X_PARAM_DISP_UPDATE_MODE_2;
  SSD168X_CHECK_IO_TX_PARAM(io, SSD168X_CMD_SET_DISP_UPDATE_CTRL,
                            (const uint8_t[]){mode}, 1, "DISP_UPDATE_CTRL");
  DMEAS_END(TAG, "[%s] took %llu us", __func__);
  return ESP_OK;
}

static esp_err_t epaper_set_lut(esp_lcd_panel_io_handle_t io,
                                const uint8_t *lut) {
  ILOG(TAG, "[%s]", __func__);
  DMEAS_START();
  SSD168X_CHECK_IO_TX_PARAM(io, SSD168X_CMD_SET_LUT_REG, lut, 153, "LUT_REG");
  SSD168X_CHECK_FUNCTION_CALL(epaper_set_display_sequence(io, lut[153]),
                              "epaper_set_display_sequence failed");

  // Use direct pointer arithmetic to avoid temporary array allocations
  SSD168X_CHECK_IO_TX_PARAM(io, SSD168X_CMD_SET_GATE_DRIVING_VOLTAGE, &lut[154],
                            1, "GATE_DRIVING_VOLTAGE");
  SSD168X_CHECK_IO_TX_PARAM(io, SSD168X_CMD_SET_SRC_DRIVING_VOLTAGE, &lut[155],
                            3, "SRC_DRIVING_VOLTAGE");
  SSD168X_CHECK_IO_TX_PARAM(io, SSD168X_CMD_SET_VCOM_REG, &lut[158], 1,
                            "VCOM_REG");
...
static esp_err_t epaper_set_ram_x_addr_counter(esp_lcd_panel_io_handle_t io,
                                               uint8_t start_x) {
  ILOG(TAG, "[%s] start_x=%02x:%hhu", __func__, start_x, start_x);
  DMEAS_START();
  SSD168X_CHECK_IO_TX_PARAM(io, SSD168X_CMD_SET_INIT_X_ADDR_COUNTER, &start_x,
                            1, "INIT_X_ADDR_COUNTER");
  DMEAS_END(TAG, "[%s] took %llu us", __func__);
  return ESP_OK;
}

static esp_err_t epaper_set_ram_y_addr_counter(esp_lcd_panel_io_handle_t io,
                                               uint8_t start_y,
                                               uint8_t start_y1) {
  ILOG(TAG, "[%s] start_y=%02x:%hhu, start_y1:%02x:%hhu", __func__, start_y,
       start_y, start_y1, start_y1);
  DMEAS_START();
  const uint8_t params[] = {start_y, start_y1};
  SSD168X_CHECK_IO_TX_PARAM(io, SSD168X_CMD_SET_INIT_Y_ADDR_COUNTER, params, 2,
                            "INIT_Y_ADDR_COUNTER");
  DMEAS_END(TAG, "[%s] took %llu us", __func__);
  return ESP_OK;
}

static esp_err_t epaper_set_cursor(esp_lcd_panel_io_handle_t io, uint8_t cur_x,
                                   uint8_t cur_y, uint8_t cur_y1) {
  ILOG(TAG, "[%s]", __func__);
  SSD168X_CHECK_FUNCTION_CALL(epaper_set_ram_x_addr_counter(io, cur_x),
                              "epaper_set_ram_x_addr_counter failed");
  SSD168X_CHECK_FUNCTION_CALL(epaper_set_ram_y_addr_counter(io, cur_y, cur_y1),
                              "epaper_set_ram_y_addr_counter failed");
  return ESP_OK;
}

static esp_err_t epaper_panel_set_sleep_ctrl(esp_lcd_panel_io_handle_t io,
                                             uint8_t sleep_mode) {
  ILOG(TAG, "[%s]", __func__);
  const uint8_t params[] = {sleep_mode};
  SSD168X_CHECK_IO_TX_PARAM(io, SSD168X_CMD_SLEEP_CTRL, params, 1,
                            "SLEEP_CTRL");
  return ESP_OK;
}
...
static esp_err_t epaper_panel_set_cursor(esp_lcd_panel_t *panel) {
  ...
  uint8_t cursor_x = mode_config->x_increase ? (p->xs_d8 + add_xx)
                                             : (p->xe_d8 + add_xx - add_x);
...
static esp_err_t epaper_set_ram_area_x(esp_lcd_panel_io_handle_t io,
                                       uint8_t start_x, uint8_t end_x) {
  ILOG(TAG, "[%s] start_x=%02x:%hhu, end_x=%02x:%hhu", __func__, start_x,
       start_x, end_x, end_x);
  DMEAS_START();
  const uint8_t params[] = {start_x, end_x};
  SSD168X_CHECK_IO_TX_PARAM(io, SSD168X_CMD_SET_RAMX_START_END_POS, params, 2,
                            "RAMX_START_END_POS");
  DMEAS_END(TAG, "[%s] took %llu us", __func__);
  return ESP_OK;
}

static esp_err_t epaper_set_ram_area_y(esp_lcd_panel_io_handle_t io,
                                       uint8_t start_y, uint8_t start_y1,
                                       uint8_t end_y, uint8_t end_y1) {
  ILOG(TAG,
       "[%s] start_y=%02x:%hhu, start_y1:%02x:%hhu end_y=%02x:%hhu, "
       "end_y1:%02x:%hhu",
       __func__, start_y, start_y, start_y1, start_y1, end_y, end_y, end_y1,
       end_y1);
  DMEAS_START();
  const uint8_t params[] = {start_y, start_y1, end_y, end_y1};
  SSD168X_CHECK_IO_TX_PARAM(io, SSD168X_CMD_SET_RAMY_START_END_POS, params, 4,
                            "RAMY_START_END_POS");
  DMEAS_END(TAG, "[%s] took %llu us", __func__);
  return ESP_OK;
}

static esp_err_t epaper_set_ram_area(esp_lcd_panel_io_handle_t io,
                                     uint8_t start_x, uint8_t end_x,
                                     uint8_t start_y, uint8_t start_y1,
                                     uint8_t end_y, uint8_t end_y1) {
  ILOG(TAG,
       "[%s] start_x=%02x:%hhu, end_x=%02x:%hhu, start_y=%02x:%hhu, "
       "start_y1=%02x:%hhu, end_y=%02x:%hhu, end_y1=%02x:%hhu",
       __func__, start_x, start_x, end_x, end_x, start_y, start_y, start_y1,
       start_y1, end_y, end_y, end_y1, end_y1);
  // --- Set RAMX/SOUCE Start/End Position
  SSD168X_CHECK_FUNCTION_CALL(epaper_set_ram_area_x(io, start_x, end_x),
                              "epaper_set_ram_area_x failed");
  // --- Set RAMY/GATE Start/End Position
  SSD168X_CHECK_FUNCTION_CALL(
      epaper_set_ram_area_y(io, start_y, start_y1, end_y, end_y1),
      "epaper_set_ram_area_y failed");
  return ESP_OK;
}
...
static esp_err_t epaper_panel_set_ram_area(esp_lcd_panel_t *panel) {
  ...
  return epaper_set_ram_area(epaper_panel->io, start_x, end_x, start_y,
                             start_y1, end_y, end_y1);
}
...
static esp_err_t epaper_panel_set_black_vram(esp_lcd_panel_io_handle_t io,
                                             const uint8_t *bw_bitmap,
                                             size_t size) {
  ILOG(TAG, "[%s]", __func__);
  DMEAS_START();
  SSD168X_CHECK_IO_TX_COLOR(io, SSD168X_CMD_WRITE_BLACK_VRAM, bw_bitmap, size,
                            "WRITE_BLACK_VRAM");
  DMEAS_END(TAG, "[%s] took %llu us", __func__);
  return ESP_OK;
}

static esp_err_t epaper_panel_set_red_vram(esp_lcd_panel_io_handle_t io,
                                           const uint8_t *red_bitmap,
                                           size_t size) {
  ILOG(TAG, "[%s]", __func__);
  DMEAS_START();
  SSD168X_CHECK_IO_TX_COLOR(io, SSD168X_CMD_WRITE_RED_VRAM, red_bitmap, size,
                            "WRITE_RED_VRAM");
  DMEAS_END(TAG, "[%s] took %llu us", __func__);
  return ESP_OK;
}

static esp_err_t epaper_panel_set_vram(esp_lcd_panel_io_handle_t io,
                                       const uint8_t *bw_bitmap,
                                       const uint8_t *red_bitmap, size_t size) {
  ILOG(TAG, "[%s] red: %d, black: %d, size: %u", __func__, bw_bitmap ? 1 : 0,
       red_bitmap ? 1 : 0, size);

  // Patch: For proper full update on SSD1680, populate both RAMs with the black
  // buffer. GxEPD2 writes to 0x26 (Previous) and 0x24 (Current).
  if (bw_bitmap && (size > 0)) {
    // Write to Current RAM (Black)
    SSD168X_CHECK_FUNCTION_CALL(
        epaper_panel_set_black_vram(io, bw_bitmap, size),
        "epaper_panel_set_black_vram failed");

    // Write to Previous RAM (Red) - using same content
    // Note: epaper_panel_set_red_vram sends CMD_WRITE_RED_VRAM (0x26)
    SSD168X_CHECK_FUNCTION_CALL(epaper_panel_set_red_vram(io, bw_bitmap, size),
                                "epaper_panel_set_red_vram failed");
  }

  if (red_bitmap && (size > 0)) {
    // If specific red buffer provided, use it. This might overwrite the above,
    // which is acceptable if user intends color.
    SSD168X_CHECK_FUNCTION_CALL(epaper_panel_set_red_vram(io, red_bitmap, size),
                                "epaper_panel_set_red_vram failed");
  }
  return ESP_OK;
}
...
static esp_err_t epaper_set_display_update_control(esp_lcd_panel_t *panel) {
  ...
  DMEAS_START();
  epaper_panel_t *epaper_panel = __containerof(panel, epaper_panel_t, base);
  // GxEPD2 uses 0x00 0x80. The previous driver sent 1 byte calculated from
  // flags. We force the 2-byte sequence that works.
  const uint8_t duc[] = {0x00, 0x80};
  if (esp_lcd_panel_io_tx_param(epaper_panel->io, SSD168X_CMD_DISP_UPDATE_CTRL,
                                duc, 2) != ESP_OK) {
    ESP_LOGE(TAG, "%s%sDISP_UPDATE_CTRL %s", drv_msg[0], drv_msg[1],
             drv_msg[2]);
    return ESP_FAIL;
  }
  DMEAS_END(TAG, "[%s] took %llu us", __func__);
...
static esp_err_t
epaper_set_active_display_update_sequence(esp_lcd_panel_io_handle_t io) {
  ILOG(TAG, "[%s]", __func__);
  DMEAS_START();
  if (esp_lcd_panel_io_tx_param(io, SSD168X_CMD_ACTIVE_DISP_UPDATE_SEQ, NULL,
                                0) != ESP_OK) {
...
static uint8_t epaper_get_ram_mode(esp_lcd_panel_t *panel) {
  ...
  if ((!(epaper_panel->_mirror_x)) && (epaper_panel->_mirror_y)) {
    mode = epaper_panel->_swap_xy ? SSD168X_PARAM_DATA_ENTRY_MODE_1
                                  : SSD168X_PARAM_DATA_ENTRY_MODE_1;
  } else if (((epaper_panel->_mirror_x)) && (!(epaper_panel->_mirror_y))) {
    // mirror x, means x is flipped
    mode = epaper_panel->_swap_xy ? SSD168X_PARAM_DATA_ENTRY_MODE_2
                                  : SSD168X_PARAM_DATA_ENTRY_MODE_2;
  } else if ((epaper_panel->_mirror_x) && (epaper_panel->_mirror_y)) {
    mode = epaper_panel->_swap_xy ? SSD168X_PARAM_DATA_ENTRY_MODE_0
                                  : SSD168X_PARAM_DATA_ENTRY_MODE_0;
    // } else {
    //     mode = epaper_panel->_swap_xy ? SSD168X_PARAM_DATA_ENTRY_MODE_3 :
    //     SSD168X_PARAM_DATA_ENTRY_MODE_3;
  }
  return mode;
}
...
static esp_err_t epaper_set_data_entry_mode(esp_lcd_panel_t *panel) {
  ...
  SSD168X_CHECK_IO_TX_PARAM(epaper_panel->io, SSD168X_CMD_DATA_ENTRY_MODE,
                            &mode, 1, "DATA_ENTRY_MODE");
  DMEAS_END(TAG, "[%s] took %llu us", __func__);
  return ESP_OK;
}

static esp_err_t epaper_set_data_entry_sequence(esp_lcd_panel_t *panel,
                                                bool setcursor) {
  ILOG(TAG, "[%s]", __func__);
  DMEAS_START();
  // set data entry mode
  SSD168X_CHECK_FUNCTION_CALL(epaper_set_data_entry_mode(panel),
                              "epaper_set_data_entry_mode failed");
  // area by cmd 0x44 0x45
  SSD168X_CHECK_FUNCTION_CALL(epaper_panel_set_ram_area(panel),
                              "epaper_panel_set_ram_area failed");
  // cursor by cmd 0x4e 0x4f
  if (setcursor) {
    SSD168X_CHECK_FUNCTION_CALL(epaper_panel_set_cursor(panel),
                                "epaper_panel_set_cursor failed");
  }
  DMEAS_END(TAG, "[%s] took %llu us", __func__);
  return ESP_OK;
}
...
static esp_err_t epaper_set_driver_output(esp_lcd_panel_t *panel) {
  ...
  DMEAS_START();
  epaper_panel_t *epaper_panel = __containerof(panel, epaper_panel_t, base);
  // Corrected for SSD1680 296 lines: 0x27 0x01 0x00
  // The previous driver logic might be defaulting to 256 or generic.
  // Overriding with GxEPD2 values: MUX=295 (296 lines)
  const uint8_t output_ctrl[] = {0x27, 0x01, 0x00};
  if (esp_lcd_panel_io_tx_param(epaper_panel->io, SSD168X_CMD_OUTPUT_CTRL,
                                output_ctrl, 3) != ESP_OK) {
...
esp_err_t epaper_panel_refresh_screen_ssd168x(esp_lcd_panel_t *panel,
                                              uint8_t update_mode) {
  ILOG(TAG, "[%s]", __func__);
  if (!panel) {
    ESP_LOGE(TAG, "%s", drv_msg[3]);
...
esp_err_t esp_lcd_new_panel_ssd168x(
  ...
  esp_lcd_ssd168x_config_t *epaper_ssd168x_conf =
  ...
  *ret_panel = &(epaper_panel->base);
  ...
  // epaper_panel->clearbuffer = epaper_ssd168x_conf->clear_img; //
  // heap_caps_malloc(img_size, MALLOC_CAP_DMA);
  // SSD168X_RETURN_ON_FALSE(epaper_panel->_clearbuffer, ESP_ERR_NO_MEM, TAG,
  // "epaper_panel_init allocating buffer memory err");
  ...
  if (epaper_panel->busy_gpio_num >= 0) {
    ...
    if (gpio_isr_handler_add(epaper_panel->busy_gpio_num,
                             epaper_driver_gpio_isr_handler, epaper_panel)) {
      ESP_LOGE(TAG, "add GPIO ISR handler err");
      goto err;
    }
    // Enable GPIO intr only before refreshing, to avoid other commands caused
    // intr trigger
    gpio_intr_disable(epaper_panel->busy_gpio_num);
  }
...
static esp_err_t epaper_panel_gpio_reset(esp_lcd_panel_t *panel) {
  ...
  if (gpio_set_level(epaper_panel->reset_gpio_num,
                     !epaper_panel->reset_level)) {
    ESP_LOGE(TAG, "gpio_set_level error");
    return ESP_FAIL;
  }
  vTaskDelay(pdMS_TO_TICKS(10));
...
esp_err_t
epaper_panel_set_bitmap_color_ssd168x(esp_lcd_panel_t *panel,
                                      esp_lcd_ssd168x_bitmap_color_t color) {
  ILOG(TAG, "[%s]", __func__);
  if (!panel) {
    ESP_LOGE(TAG, "%s", drv_msg[3]);
    return ESP_ERR_INVALID_ARG;
...
esp_err_t epaper_panel_set_next_init_mode_ssd168x(
    esp_lcd_panel_t *panel, epaper_panel_init_mode_t next_init_mode) {
  ILOG(TAG, "[%s]", __func__);
  epaper_panel_t *epaper_panel = __containerof(panel, epaper_panel_t, base);
...
esp_err_t epaper_panel_set_next_sleep_mode_ssd168x(
    esp_lcd_panel_t *panel, epaper_panel_sleep_mode_t next_sleep_mode) {
  ILOG(TAG, "[%s]", __func__);
  epaper_panel_t *epaper_panel = __containerof(panel, epaper_panel_t, base);
...
static esp_err_t epaper_panel_init_stage_2(esp_lcd_panel_t *panel) {
  ...
  DMEAS_START();
  epaper_panel_t *epaper_panel = __containerof(panel, epaper_panel_t, base);
  set_ram_params(epaper_panel, 0, 0, epaper_panel->width, epaper_panel->height,
                 epaper_get_ram_mode(panel), false); // default
...
static esp_err_t epaper_panel_init_stage_3(esp_lcd_panel_t *panel,
                                           const uint8_t *lut) {
  ILOG(TAG, "[%s]", __func__);
  DMEAS_START();
  epaper_panel_t *epaper_panel = __containerof(panel, epaper_panel_t, base);
  ...
  if (esp_lcd_panel_io_tx_param(
          epaper_panel->io, SSD168X_CMD_SET_BORDER_WAVEFORM,
          (const uint8_t[]){SSD168X_PARAM_BORDER_WAVEFORM_0}, 1) != ESP_OK) {
    ESP_LOGE(TAG, "%s%sBORDER_WAVEFORM %s", drv_msg[0], drv_msg[1], drv_msg[2]);
    return ESP_FAIL;
  }

  // --- Temperature Sensor Control
  if (esp_lcd_panel_io_tx_param(epaper_panel->io, SSD168X_CMD_SET_TEMP_SENSOR,
                                (const uint8_t[]){SSD168X_PARAM_TEMP_SENSOR},
                                1) != ESP_OK) {
  ...
  if (lut) {
    ...
    if (esp_lcd_panel_io_tx_param(epaper_panel->io, 0x37,
                                  (const uint8_t[]){0x00, 0x00, 0x00, 0x00,
                                                    0x00, 0x40, 0x00, 0x00,
                                                    0x00, 0x00},
                                  10) != ESP_OK) {
      return ESP_FAIL;
    ...
    if (epaper_set_display_sequence(epaper_panel->io,
                                    epaper_panel->next_init_mode) != ESP_OK) {
      return ESP_FAIL;
    }
  }
...
static esp_err_t epaper_panel_init_stage_4(esp_lcd_panel_t *panel,
                                           uint8_t color) {
  ILOG(TAG, "[%s]", __func__);
  DMEAS_START();
  // --- Set LUT
  epaper_panel_t *epaper_panel = __containerof(panel, epaper_panel_t, base);
  // esp_lcd_panel_io_handle_t io = epaper_panel->io;
  // ESP_RETURN_ON_ERROR(epaper_set_lut(io, SSD168X_LUT_DEFAULT), TAG,
  // "epaper_set_lut error"); ESP_RETURN_ON_ERROR(set_cursor(epaper_panel), TAG,
  // "epaper_set_cursor() error"); clear screen
  if (!(epaper_panel->_non_copy_mode)) {
    memset(epaper_panel->_framebuffer, color, epaper_panel->_framebuffer_size);
    if (epaper_panel_set_vram(epaper_panel->io, (epaper_panel->_framebuffer),
                              (epaper_panel->_framebuffer),
                              epaper_panel->_framebuffer_size)) {
      return ESP_FAIL;
    }
  }
  DMEAS_END(TAG, "[%s] took %llu us", __func__);
...
static esp_err_t epaper_panel_init_stage_5(esp_lcd_panel_t *panel) {
  ...
  // SSD168X_CMD_SET_END_OPTION, (const uint8_t[]) {
...
esp_err_t
epaper_panel_init_screen_ssd168x(esp_lcd_panel_t *panel,
...
esp_err_t epaper_panel_clear_screen_ssd168x(esp_lcd_panel_t *panel,
                                            uint8_t *color_data,
                                            uint8_t color) {
  epaper_panel_t *epaper_panel = __containerof(panel, epaper_panel_t, base);
  ILOG(TAG, "[%s] color: %02x panelw: %hd panelh: %hd", __func__, color,
       epaper_panel->width, epaper_panel->height);
  DMEAS_START();
  ram_params_t *p = &(epaper_panel->_ram_params);
  set_ram_params(epaper_panel, 0, 0, epaper_panel->width, epaper_panel->height,
                 epaper_get_ram_mode(panel), false);
  // --- Set cursor & data entry sequence
  if (epaper_set_data_entry_sequence(panel, true)) {
    return ESP_FAIL;
  }
  memset(color_data, color, p->buffer_size);
  if (epaper_panel_set_vram(epaper_panel->io, (uint8_t *)(color_data),
                            (color_data), p->buffer_size)) {
    return ESP_FAIL;
  }
...
static esp_err_t epaper_panel_draw_bitmap(esp_lcd_panel_t *panel, int x_start,
                                          int y_start, int x_end, int y_end,
                                          const void *color_data) {
  ILOG(TAG, "[%s] start bounds: {x:%d, y:%d} -> {x:%d, y:%d}", __func__,
  ...
  // if (epaper_panel->_non_copy_mode) {
  //     SSD168X_RETURN_ON_FALSE(!(epaper_panel->_swap_xy), ESP_ERR_INVALID_ARG,
  //     TAG, "swap-xy is unavailable when enabling non-copy mode");
  //     SSD168X_RETURN_ON_FALSE(!(epaper_panel->_mirror_y),
  //     ESP_ERR_INVALID_ARG, TAG, "mirror_y is unavailable when enabling
  //     non-copy mode");
  // }
  if (!color_data) {
  ...
  ram_params_t *p = &(epaper_panel->_ram_params);
  set_ram_params(epaper_panel, x_start, y_start, x_end, y_end,
                 epaper_get_ram_mode(panel), epaper_panel->_swap_xy);
  // p->ram_mode = epaper_get_ram_mode(panel);
  DLOG(TAG, "[%s] converted bounds: {x:%d, y:%d} -> {x:%d, y:%d}", __func__,
       x_start, y_start, x_end, y_end);
  ...
  if (epaper_panel->_non_copy_mode) {
    ...
    if (!esp_ptr_dma_capable(epaper_panel->_framebuffer)) {
      ESP_LOGW(TAG, "Bitmap not DMA capable, use DMA capable memory to avoid "
  ...
  uint8_t *data =
      (uint8_t *)(!epaper_panel->_non_copy_mode ? epaper_panel->_framebuffer
                                                : color_data);
  // if (epaper_panel->bitmap_color == SSD168X_EPAPER_BITMAP_BLACK) {
  //    if(epaper_panel_set_vram(epaper_panel->io, (uint8_t
  //    *)(!epaper_panel->_non_copy_mode ? epaper_panel->_framebuffer :
  //    color_data), NULL, p->buffer_size) {
  //        return ESP_FAIL;
  //    }
  ...
  // tx_param will wait until DMA transaction finishes, so it is safe to call
  // panel_epaper_refresh_screen at once. The driver will not call the
  // `epaper_panel_refresh_screen` automatically, please call it manually.
  return ESP_OK;
}

static esp_err_t epaper_panel_invert_color(esp_lcd_panel_t *panel,
...
static esp_err_t epaper_panel_mirror(esp_lcd_panel_t *panel, bool mirror_x,
                                     bool mirror_y) {
  ILOG(TAG, "[%s] %s %s", __func__, mirror_x ? "mirror_x" : "",
       mirror_y ? "mirror_y" : "");
  epaper_panel_t *epaper_panel = __containerof(panel, epaper_panel_t, base);
  // if (mirror_y) {
  //     if (epaper_panel->_non_copy_mode) {
  //         ESP_LOGE(TAG, "mirror_y is unavailable when enabling non-copy
  //         mode"); return ESP_ERR_INVALID_ARG;
  //     }
  // }
...
static esp_err_t epaper_panel_swap_xy(esp_lcd_panel_t *panel, bool swap_axes) {
  ...
  // if (swap_axes) {
  //     if (epaper_panel->_non_copy_mode) {
  //         ESP_LOGE(TAG, "swap_xy is unavailable when enabling non-copy
  //         mode"); return ESP_ERR_INVALID_ARG;
  //     }
  // }
...
static esp_err_t epaper_panel_set_gap(esp_lcd_panel_t *panel, int x_gap,
                                      int y_gap) {
  ILOG(TAG, "[%s]", __func__);
  epaper_panel_t *epaper_panel = __containerof(panel, epaper_panel_t, base);
...
// static void rotate(uint8_t *img, uint8_t *fb, int width, int height, uint16_t
// rotation) { #if defined(CONFIG_LCD_ENABLE_DEBUG_LOG)
//     ILOG(TAG, "[%s]", __func__);
// #endif
//     switch (rotation) {
//         case 0:
...
void rotate_bitmap(unsigned char *src, unsigned char *dest, int width,
                   int height, unsigned char rotation,
                   pixel_convert_cb_t convert_cb, int data_format,
                   void *user_data) {
  ILOG(TAG, "[%s] w:%d h:%d rotation:%d convert_cb:%p", __func__, width, height,
       rotation, (void *)convert_cb);
  ...
  for (int src_y = 0; src_y < height; src_y++) {
    for (int src_x = 0; src_x < width; src_x++) {
      ...
      if (convert_cb == NULL) {
        ...
        pixel = convert_cb(
            src, (esp_lcd_ssd168x_area_t){src_x, src_y, width, height},
            rotation, data_format, user_data);
      }
      ...
      if (dest_x >= 0 && dest_x < dest_width && dest_y >= 0 &&
          dest_y < dest_height) {
...
static esp_err_t process_bitmap(esp_lcd_panel_t *panel,
                                const void *color_data) {
  epaper_panel_t *epaper_panel = __containerof(panel, epaper_panel_t, base);
  ram_params_t *p = &(epaper_panel->_ram_params);

  ILOG(TAG, "[%s] w:%hd h:%hd buffer_size:%u", __func__, p->w, p->h,
       p->buffer_size);
  DLOG(TAG, "mirror_x:%d mirror_y:%d swap_xy:%d", epaper_panel->_mirror_x,
       epaper_panel->_mirror_y, epaper_panel->_swap_xy);
  ...
  if (swap_xy) {
    ...
    rotate_bitmap((uint8_t *)color_data, epaper_panel->_framebuffer, p->h, p->w,
                  3, NULL, 0, NULL);
