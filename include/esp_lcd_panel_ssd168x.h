 ...
 * when rotating bitmaps, keeping the driver independent of specific libraries
 * like LVGL.
 *
 ...
 */
typedef uint8_t (*pixel_convert_cb_t)(const unsigned char *src_data,
                                      esp_lcd_ssd168x_area_t src_area,
                                      int rotation, int data_format,
                                      void *user_data);
...
typedef struct {
  ...
  // const unsigned char * clear_img; /*!< Data to be written to clear the
  // screen */
} esp_lcd_ssd168x_config_t;

/**
 * @brief Enum of colors available of ssd168x e-paper
 *        Some of the ssd168x e-paper panels support not only black pixels but
 ...
 */
esp_err_t
esp_lcd_new_panel_ssd168x(const esp_lcd_panel_io_handle_t io,
 ...
 *
 * @note The SSD168X has two separate vrams. One for black and the other for
 ...
 *
 * @param[in] panel LCD panel handle
 * @param[in] color a enum value, SSD168X_EPAPER_BITMAP_BLACK or
 ...
 * @param[in] size size of your lut array, make sure it is SSD168X_LUT_SIZE
 * bytes
 * @return  ESP_OK                on success
 *          ESP_ERR_INVALID_ARG   if parameter is invalid
 */
...
typedef enum {
  /// Display Update Control 2 : Display Update Sequence Option:  Enable the
  /// stage for Master Activation
  /// Enable clock signal, Enable Analog
  INIT_MODE_PARTIAL = 0xc0,
  /// Enable clock signal, Enable Analog,  Display with DISPLAY Mode 1,  Disable
  /// Analog,  Disable OSC
  INIT_MODE_FAST_1 = 0xc7,
  /// Enable clock signal,  Enable Analog,  Display with DISPLAY Mode 2, Disable
  /// Analog, Disable OSC
  INIT_MODE_FAST_2 = 0xcf,
  /// Enable clock signal, Enable Analog, Load temperature value, DISPLAY with
  /// DISPLAY Mode 1, Disable Analog, Disable OSC
  INIT_MODE_FULL_1 = 0xf7,
  /// Enable clock signal, Enable Analog, Load temperature value, DISPLAY with
  /// DISPLAY Mode 2, Disable Analog, Disable OSC
